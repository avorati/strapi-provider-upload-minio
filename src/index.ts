...
class MinioProvider implements StrapiProvider {
  ...
  private readonly config: Required<
    Omit<ProviderOptions, "useSSL" | "private">
  > & {
    useSSL: boolean;
    private: boolean;
  };
  ...
  private normalizeConfig(options: ProviderOptions): Required<
    Omit<ProviderOptions, "useSSL" | "private">
  > & {
    useSSL: boolean;
    private: boolean;
  ...
  public async upload(file: StrapiFile): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.client
        .putObject(this.config.bucket, uploadPath, content, file.size, metadata)
        .then(() => {
  ...
  public async delete(file: StrapiFile): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.client
          .removeObject(this.config.bucket, filePath)
          .then(() => {
            resolve();
          })
          .catch((error: Error) => {
  ...
  public async getSignedUrl(file: StrapiFile): Promise<SignedUrlResponse> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.client
          .presignedGetObject(this.config.bucket, filePath, this.config.expiry)
          .then((presignedUrl: string) => {
            resolve({ url: presignedUrl });
          })
          .catch((error: Error) => {
            reject(
              new Error(`Failed to generate signed URL: ${error.message}`)
            );
          });
      } catch (error) {
        reject(
          new Error(
            `Failed to process signed URL request: ${
              error instanceof Error ? error.message : "Unknown error"
            }`
          )
        );
